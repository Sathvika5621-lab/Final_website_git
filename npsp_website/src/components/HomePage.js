...
const HomePage = ({ mousedetails }) => {
    ...
    const handleFormsClick = (rowData) => {
        navigate('/formslist', { state: { mouseDetails: rowData } });
    };

    const handleSyringePrepClick = () => {
        navigate('/syringe-prep', { state: { selectedMice } });
    };
    ...
    const actionTemplate = (rowData) => (
        <div className='button-container'>
             <Button label="Forms" onClick={() => handleFormsClick(rowData)} />
    ...
    return (
        <div className='p-grid'>
            ...
            <div className="card" style={{ maxWidth: '1500px', margin: '0px auto', padding: '3px' }}>
                <DataTable value={mousedetails} paginator rows={5} rowsPerPageOptions={[5, 10, 25, 50]} stripedRows tableStyle={{ minWidth: '30rem' }} filters={filters} globalFilterFields={['mouse_id', 'site', 'sex', 'dateofbirth', 'shipmentdate', 'cageid', 'slurrydose', 'endpoint', 'studytype']} header={header} selection={selectedMice} onSelectionChange={(e) => setSelectedMice(e.value)} selectionMode="multiple">
